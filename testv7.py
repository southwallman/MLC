...
def test_with_best_params_from_json_ai(
        ...
        test_loader=None,  # 🚨 改为直接接收全局建好的 DataLoader
        output_dir='.',
        label_list=None,
        label_image=False,
    ...
    if model_save_path is not None and os.path.exists(model_save_path):
        ...
        del checkpoint  # 🚨 极度关键：加载完立刻销毁中间态变量，释放显存！
    else:
    ...
    with torch.no_grad():
        for images_batch, targets_batch, paths_batch in tqdm(test_loader, desc=f"测试 {name}"):
            ...
            for i in range(len(paths_batch)):
                path = paths_batch[i]
                true = targets_batch[i].cpu().numpy()
                pred = preds[i]
                ...
                if error_labels:
                    error_records.append({
                        ...
                        'probs': probs[i], 'error_labels': error_labels
                    })
...
def config():
    ...
    raise ValueError(f"不支持的 RUN_MODE: {RUN_MODE}")
